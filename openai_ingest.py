#!/usr/bin/env python3
"""
OpenAI-based Ingestion Script for Physical AI & Humanoid Robotics Book
Uses OpenAI embeddings instead of Cohere for better stability and rate limits
"""
import os
import requests
...
# Configure OpenAI
openai.api_key = os.getenv('OPENAI_API_KEY')  # or use OPENROUTER_API_KEY if you prefer OpenRouter

class OpenAIBookIngestor:
    def __init__(self):
        ...
        # Use OpenAI embeddings (1536 dimensions for text-embedding-3-small)
        self.embedding_model = "text-embedding-3-small"  # or "text-embedding-3-large" for higher quality
        self.embedding_dimensions = 1536  # For text-embedding-3-small (768 for large)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "rate_limit_exceeded" in str(e).lower() or "429" in str(e):
                    ...
                    logger.warning(f"OpenAI rate limit hit (attempt {attempt + 1}): {e}. Waiting {wait_time}s...")
    ...
    def ingest_urls(self, urls: List[str], chunk_size: int = 512, overlap: int = 25):
        """Ingest multiple URLs into Qdrant"""
        logger.info(f"Starting ingestion of {len(urls)} main lesson URLs using OpenAI embeddings...")
...
def main():
    ...
    import sys
    if len(sys.argv) < 2:
        print("Usage: python openai_ingest.py <urls_file.txt>")
        sys.exit(1)
